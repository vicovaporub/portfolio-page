...
export default function NavBar() {
  ...
  return (
    <header className="z-[999] relative">
     ...
     rounded-none border border-gray-700 dark:border-gray-300 border-opacity-40 
     dark:bg-opacity-60 bg-white dark:bg-gray-950 bg-opacity-80 shadow-lg
      shadow-black/[0.03] backdrop-blur-[0.5rem] sm:top-6 sm:h-[3.25rem]
       sm:w-[32rem] sm:rounded-full"
        initial={{ y: -100, x: "-50%", opacity: 0 }}
        animate={{ y: 0, x: "-50%", opacity: 1 }}
      ></motion.div>
      <nav
        className="flex fixed top-[0.15rem] 
      left-1/2 h-12 -translate-x-1/2
      ...
      >
        ...
        justify-center gap-y-1 text-[0.9rem] font-medium
         text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-3"
        >
          {data.map((link) => (
            <motion.li
            ...
            >
              <Link
                className="flex w-full items-center justify-center px-3 py-3
                 dark:text-gray-300 dark:hover:text-gray-100 hover:text-gray-950 
                 transition"
                href={link.hash}
              >
                {link.name}
